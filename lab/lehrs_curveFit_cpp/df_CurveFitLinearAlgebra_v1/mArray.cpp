    ...
    for(int r=0;r<Rows;r++){
        for(int c=0;c<Cols;c++){
                ...
                if(cd>0 && c==1){
                    ...
                    //cout<<"["<<r<<","<<c<<"]="<<Data[r][c]<<" sumYE=";
                    sumYE+=Data[r][c];
                    //cout<<" sumYE="<<fixed<<setprecision(2)<<sumYE<<" ";
                }
        }//cout<<endl;
    }
    //cout<<"sumX="<<sumX<<"\nsumY="<<fixed<<setprecision(2)<<sumY<<"\nsumYE="<<sumYE<<" ";
    //cout<<"\neof...\n\n";
    in.close();
}

double getThis(mArray &right){
    //double temp=0;
    
//    for(int r=0;r<right.getRows();r++){
//        for(int c=0;c<right.getCols();c++){
//            //cout<<left.getData(r,c);
//        }
//    }
    return 1.0;
}
