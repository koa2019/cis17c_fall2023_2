...
void setTopPlyrs(map<string,float>&,unordered_set<string>&);
void topPlyrs(map<string,float>&,unordered_set<string>&);
void pause();
...
void concepts(){
    ...
    // Sets, lists, maps 
    // Initialize unordered map with pair(hiScores,scores)
    std::map<string, float> hiScores; 
    topPlyrs(hiScores,names);
    pause();
...
void topPlyrs(map<string, float> &hiScores, unordered_set<string> &names){ 
    
    // Initialize unordered map with pair(hiScores,scores)
    //std::map<string, float> hiScores; 
    setTopPlyrs(hiScores,names);    
   
    
    // Print map (key,value) pairs of high scores
    cout<<"Map Sorted Alphabetically:  \n";
    cout<<"Map pair(set<string>, list<float>)\n";
    prntMap(hiScores);// Print unordered map (key,value) pairs of hiScores & high scores
    
    // Find max_element
    //https://www.techiedelight.com/find-element-with-the-maximum-value-in-a-map-in-cpp/
    auto maxPair = std::max_element(hiScores.begin(), hiScores.end(), [](const auto &x, const auto &y) {
    ...
    //            maxScore=(maxScore<currntMax)?currntMax:maxScore;
...
void setTopPlyrs(map<string, float> &hiScores, unordered_set<string> &names){        
...
// Print unordered map (key,value) pairs of hiScores & high scores
