...
string pickP2Name(unordered_set<string> &);
...
void menu(){   
    char ans,ch;
    cout<<"Dr. Lehr,\n\n"
        <<"Option 1:\nDemonstrates how I applied CIS-17C concepts \nto my CIS-5 "
        <<"Battleship game.\nHowever, I was not able to fully blended them\n"
        <<"into my game without breaking it.\n\n"
        <<"Option 2:\nIs a complete working game but it still utilizes char 2D arrays.\n\n\n\n"
        <<"\t\tMenu\n\n";
    
      do {         
        cout << "Press 1: To see CIS 17C concepts applied to this game.\n"
             << "Press 2: To start game.\n"
             << "Press any other key to exit.\n";
        cin  >> ch;
        switch(ch){       
            case '1': concepts(); break;
            case '2': play(); break;
...
void concepts(){
    ...
    cout<<"Combined 2 sets and then inserted "<<p1Name<<" on top.\n";
    cout<<"Set Unsorted Players: \n";
    prntStrSet(names);    
    pause();
    name2 = getMin(names);
    cout<<setw(16)<<p1Name<<" vs "<<name2<< "!\n\n";  
    pause();
    // Sort & search names   
    topPlyrs(names);
    pause();
...
string getMin(unordered_set<string> &names){        
    ...
    cout<<"Shortest of \""<< *itr<<"\" and \""<<last<<"\" is \""<< min<< "\"\n\n";
    return min;
}
...
// randomly picks a name from an array as player 2's name
string pickP2Name(unordered_set<string> &names){    
    cout << "\nLocating your opponent online......\n";    
    ////shuffleThis();
...
void topPlyrs(unordered_set<string> &names){ 
    ...
    cout<<"Map pair(set<string>name, list<float>scores).\n";
    ...
    if(it==topPlyrs.end()){
       cout<<"Sorry, "<<sName<<" wasn't found.\n";
        ...
        cout << (*it).first <<" is in the "
             << count<<" spot for this week's top player\nwith "
             << (*it).second <<" points.\n\n";
    }    
...
map<string, float> setTopPlyrs(unordered_set<string> &names){  
    ...
    //ostream_iterator<float> cout_it(cout, " "); // Get output stream iterators
    cout<<"set:\n";
