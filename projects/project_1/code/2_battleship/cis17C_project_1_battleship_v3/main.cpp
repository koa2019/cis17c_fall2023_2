 ...
 *  
 * Version 3: 
 * Created shuffleThis(). It works on arrays, but not on a set container,
 * so I created getMin() and used std::min() to find the shortest name 
 * between 2 strings and then set player 2’s name in main().
 * Implement min algorithm
...
void conceptsMenu(unordered_set<string>&,string, string);
...
int main(int argc, char** argv) {
    ...
    conceptsMenu(names,p1Name,name2);    
...
void conceptsMenu(unordered_set<string> &names,string p1Name, string name2){
        cout << "\nSet Unsorted: Players \n";
        prntStrSet(names);
        p1Name=getName();  // Set player 1's name to uppercase case 
        name2 = getMin(names);
