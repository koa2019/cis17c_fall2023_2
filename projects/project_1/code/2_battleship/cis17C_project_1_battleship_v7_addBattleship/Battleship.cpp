...
void Battleship::play(int a, int &rnd){
   ...
   /**********************  START THIS PLAYER'S TURN  *************************/
    ...
    while(!(numRolls == MAXSHIPS)){

        
        cout<< "\n   "<<name<<"'s turn. Round "<<rnd+1<<" Roll "<<numRolls+1<< endl;
        pause();                 // Pause to mimic rolling dice
        ...
        if(numRolls == 3){ ans = 2; } // if last roll, force switch case 2 to run
        else { displayOptions(ans); } // else offer them options to choose from

        
        switch(ans){ // choose dice to keep, pick a category, re-roll or exit game
            ...
            {
                numRolls = MAXSHIPS; // reassign it, so while(numRolls<maxRolls) stops
...
//*****************************************************************
void Battleship::selectDice(int a) {
