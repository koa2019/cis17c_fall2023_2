...
// https://www.geeksforgeeks.org/quicksort-on-singly-linked-list/?ref=ml_lbp
template<class T>
void LinkedList<T>::quick_sort(T &first, T &last)
{                            //(struct Node* first, struct Node* last) 
    setTail();
    if (first == last) { 
        return; 
    } 
    temp = partition(first, last); 
    cout<<temp<<endl;
//    if (temp != NULL && pivot->next != NULL) { 
//        quick_sort(temp->linkPtr, last); 
//    } 
//  
//    if (pivot != NULL && first != temp) { 
//        quick_sort(first, temp); 
//    } 
} 
...
// position 
template<class T>
T LinkedList<T>::partition(T &first, T &last) 
{ 
