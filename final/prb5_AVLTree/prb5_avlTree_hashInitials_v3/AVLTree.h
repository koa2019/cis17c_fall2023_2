...
class AVLTree{
    public:
        ...
        int diff(BNTnode *);             //Difference of right/left sub-trees
        AVLTree(){root = NULL; nodeCount=0;}          //Constructor
        ...
        BNTnode *balance(BNTnode *);     //Balance sub-trees with diff > 1
        ...
        void prntLev(BNTnode *tree); //Output the Print Level
        void prntLev(BNTnode *tree, int); //Output the Print Level
        bool AVLsearch(BNTnode *,int);
};

 bool AVLTree::AVLsearch(BNTnode *root, int key) {
...
//******************************************************************************
void AVLTree::prntLev(BNTnode *tree) {
    int h = height(tree);//(tree->left);
    for (int i = 1; i <= h; i++)
    {        
        cout<<"level "<<i<<endl;
        prntLev(tree, i);//(tree->left, i);
        cout<<endl;
    }
}
...
//******************************************************************************
void AVLTree::prntLev(BNTnode *tree, int level) {
    if (tree == NULL)return;
    if (level == 1)
        cout << tree->data << " ";
    else if (level > 1) {
        prntLev(tree->left, level-1);
        prntLev(tree->right, level-1);
    }
}
...
void AVLTree::inorder(BNTnode *tree){
    ...
    //cout<<"Recursive call Left. count = "<<count<<endl;
    inorder (tree->left);
    
    /* then print the data of node */
    //cout<<"Data at Root ";
    cout<<setw(3)<<tree->data;
    ...
    //cout<<"Recursive call Right. count = "<<count<<endl;
    inorder (tree->right);
}
...
void AVLTree::postorder(BNTnode *tree){
    if (tree == NULL) return;
    // first recur on left sub-tree
    postorder ( tree ->left );
     // then recur on right sub-tree
...
//******************************************************************************
//Helper function to find the maximum value node in the sub-tree rooted at `ptr`
//******************************************************************************
...
void AVLTree::deleteBNT(BNTnode* &root, int key)
{
	...
	// if the given key is less than the root node, recur for the left sub-tree
	if (key < root->data) {
		deleteBNT(root->left, key);
	}

	// if the given key is more than the root node, recur for the right sub-tree
