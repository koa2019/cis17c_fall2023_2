 ...
 * Purpose:  cis 17c final prb 5 avl tree vs hash v2
 * Code is based off AVL_binaryTree_3 and combines final>prb1_hash_v3
 * 
 * v2:
 * Only pushed hash into AVL tree. Not index = hash%512.
 * search for random string and return number of nodes it took 
 * to find it using hash versus AVL.
 * 
 * To do: 
 * 
...
int main(int argc, char** argv) {
    ...
    int rNum = rand()%indxUsed1x.size();
    int findNode = indxUsed1x[rNum]; //rNum;//
    cout<<endl<<endl;
    cout<<"Number of Hashes: "<<vecStr.size()<<endl<<endl;
    cout<<"Searching hash for node: "<< findNode <<endl;    
    int findIndx = 0;
    for(int i=0;i<SIZE;i++){
        findIndx = sv[i].find(findNode);
        if(findIndx == 0){
    ...
    if(findIndx != 0){cout << findNode << " was not found. Searched ";} 
