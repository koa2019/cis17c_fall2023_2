...
void MySTL::start(){          
    ...
    std::stack<float> tempRad;
    std::stack<float> radians1;
    std::stack<float> radians2;
    tempRad=stack;
    radians1=stack;
    radians2=stack;
    ...
    //*************************************************************************
    // Set sinh() and h() values and counts
    //*************************************************************************
    ...
    while(radians1.empty()==false){
        ...
        //angDeg = radians1.top();  // Get top value in stack     
        //angRad=angDeg*atan(1)/45;// Convert degree radians
        angRad = radians1.top();
        ysin[i].yVal   = h(angRad);
        ysin[i].hCount = cntSinH;
        ysin[i].gCount = cntCosG;

//        cout<<setw(2)<<"sinh("<<setw(5)<<setprecision(2)<<angRad<<")  "     
//            <<setw(7)<<setprecision(4)<<ysin[i].yVal<<setw(3)<<" " 
//            //<<setw(7)<<cosVal<<"  "
//            //<<setw(7)<<cosG<<"  "
//            <<setw(7)<<ysin[i].hCount<<"   "// total count for h() being called
//            <<setw(7)<<ysin[i].gCount<<"  "<<endl; // total count for g() being called<<endl;                 
        radians1.pop(); // delete top value in stack
        cntSinH = cntCosG = hCount1 = gCount1 = 0;
        ++i;
    }    
    //cout<<"i = "<<i<<endl;
    ...
    sinH = cosG = cntSinH = cntCosG = hCount1 = gCount1 = 0;
    i=0;
    while(radians2.empty()==false){
        
        angRad = radians2.top();
        ycos[i].yVal   = g(angRad);
        ycos[i].hCount = cntSinH;
        ycos[i].gCount = cntCosG;
