/* Author: Danielle
 * Created: 9-21-23 @445pm
 * Purpose:  Abstraction, Operator Overloading, 
 *           Copy Construction and Polymorphism
 * 
 * v2: defines and implements tab3 by defining PlusTab operator+().
...
// Student to supply
// https://discord.com/channels/1143992015646900355/1143992016414462018/1154273739358339113
PlusTab PlusTab::operator+(const PlusTab &right){
   
    PlusTab temp(right);// Calls PlusTab Table copy constructor to 
    int sum;
    ...
    for(int c=0;c<szCol;c++){
        for(int r=0;r<szRow;r++){
            sum=0;
            sum=getData(r,c)+temp.getData(r,c);
            //cout<<sum<<" ";
            temp.setData(r,c,sum);// Calls Tables setData()
        } //cout<<endl;
    }//cout<<endl;
    return temp;
}
...
int main(int argc, char** argv) {
   ...
   //Test out the Overloaded Tables constructors from different inherited classes
   PlusTab tab1(rows,cols);// Calls PlusTab Table constructor to create **columns
   PlusTab tab2(tab1);// Calls PlusTab Table copy constructor to 
   PlusTab tab3=tab1+tab2;// Calls PlusTab operator+()
