 ...
 * Purpose:  Template aggregates v3
 * 
...
Table<T> Table<T>::operator+(const Table<T> &right){
    ...
    // Copy object on the right side of plus sign
    Table<float> temp(right); 
    
    // Loop through the object and add the left object's value to the right object's values
    for(int col=0;col<right.getSzCol();col++){
        for(int row=0; row<right.getSzRow(); row++){
            temp.setData(row,col, getData(row,col)+temp.getData(row,col));
        }
    }
    return temp;
}
