 ...
 * 
 * Version 2: Uses the vector syntax in main() as an example of what the
 * 4 template functions are supposed to do. You have to manually code them because
 * the template uses a pointer arrays and NOT vectors.
 */
...
public:
   ...
   void pshFrnt(T);
   void pshBack(T);
   void prntTempArr(SimpleVector &);
   void resetAPtr(SimpleVector &);
   T popFrnt();
   T popBack();
};
...
void SimpleVector<T>::pshFrnt(T val) 
{
    ...
    resetAPtr(tempPtr);
    //SimpleVector<char> aptr(tempPtr);   
