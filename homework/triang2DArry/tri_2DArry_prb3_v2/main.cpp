 ...
 * Author: Danielle F
 * Created: 9-3-23
 * Purpose:  Triangular array v2
 *           Complete the functions with the defined structure
 
 * v2:
 * wrote code for TriMatx *fillMat(int)
 * To do:
 * write code for void mrkSort(TriMatx *)
...
void prntAry(TriMatx *triMt){
    for(int i=0;i<triMt->size;i++){
        cout<<triMt->col[i]<<" "; //Confirming col[] was saved correctly
        for(int j=0;j<triMt->col[i]; j++){
            cout<<triMt->data[i][j]<<" ";
        }
        cout<<endl;
...
//Create this function.
//Sort the columns with an index
