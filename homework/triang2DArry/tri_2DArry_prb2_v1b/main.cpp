...
int **fillAry(int *col,int rows){
    ...
    mrkSort(col,indx,rows);
    //prntAry(indx,rows);
    //int indx2[]={1,2,2,3,4,5,6,7,8,9}; // static array mimics mrkSort does to indx[] 
    for(int i=0;i<rows;i++){
        for(int j=0;j<col[indx[i]];j++){ //col[indx[i]]  //rows
...
//      col   -> Column array size n with number of columns in each row
//      array -> 2D Triangular Array/Matrix
void prntAry(int **trangl,int *col,int *indx,int rows){
    for(int i=0;i<rows;i++){
        for(int j=0;j<col[indx[i]];j++){ // rows
            cout<<trangl[i][j]<<" "; // prints unsorted array
        }
        cout<<endl;
    }
    cout<<endl<<endl;
