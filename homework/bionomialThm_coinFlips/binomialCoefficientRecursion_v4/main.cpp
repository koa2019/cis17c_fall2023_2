 ...
 * Created: 2023 
 * Purpose: Bias Coin flip Binomial Coefficient v4
 * 
...
int main(int argc, char** argv){
    ...
    int nFlips=1,       
        ...
        m = 1, // number of possible heads
        n = 2;//number of coins multiplied by faces on 1 coin       
    float bias = .60;// coin bias. Normally 50%    
         
    getResults(n,m,nFlips,.5);  
    cout<<endl<<endl;  
...
//Function Implementations
 void getResults(int n, int m, int nFlips, float bias){
    ...
    float problty, // probability formula 
          problty_a, // probability formula 
          problty_b, // probability formula 
          sumProblty = 0;
    ...
    for (int i = 0; i <= nFlips; i++) {
        ...
        problty_a = pow(bias,n);
        problty_b = pow((1-bias),n-m); //probability    
        problty = problty_a * problty_b;
        sumProblty += problty;
    ...
    cout<<"Binomial Equation using Factorial Recursion formula:\n"
        ...
        << " = " << binoCoef 
        << " * " << setprecision(3) << problty_a 
        << " * " << setprecision(2) << problty_b
        << " = " << setprecision(1) << binoCoef*sumProblty*100 <<"%\n";
        //<< " = " << binoCoef << " * " << setprecision(3) << sumProblty 
        //<< " = " << setprecision(1) << binoCoef*sumProblty*100 <<"%\n";
